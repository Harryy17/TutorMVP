...
export default function SessionSidebar({
  ...
  return (
    <aside
      className={`relative flex flex-col h-screen transition-all duration-300 z-30 select-none ${
      ...
      <div className="p-3 border-b border-[var(--paper-rule)] flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-[var(--ink-soft)] hover:text-[var(--ink)] hover:bg-[var(--sage-soft)] transition-colors ml-auto"
          >
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 rounded-xl transition-all duration-150 font-medium ${
            isCollapsed
        ...
        {sessions.map((s) => {
          ...
          return (
            <div
              ...
              className={`group relative flex items-center gap-2.5 p-2 rounded-xl cursor-pointer transition-all duration-150 ${
                isActive
                  ? 'bg-[#EBF1ED] border border-emerald-300/80 shadow-xs'
                  : 'hover:bg-[var(--sage-soft)] border border-transparent'
              }`}
            >
              {/* Left Indicator Icon */}
              <div
              ...
              {/* Delete Button (Expanded only, on hover) */}
              {!isCollapsed && (
                <button
                  onClick={(e) => handleDelete(e, s.session_id)}
                  disabled={deletingId === s.session_id}
                  title="Delete Session & Database"
                  className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-red-100 text-slate-400 hover:text-red-600 transition-all flex-shrink-0"
                >
